Template.SeeDevUxorders.helpers({
	//Filtrando todas as orders
	uxorder: function(){
		return Uxorders.find({developer: Meteor.userId()}, { sort: { createdAt: 'desc' }});
	},
	checkSomeUxorder: function(verify){
		var uxo = Uxorders.find({developer: Meteor.userId()}, { sort: { createdAt: 'desc' }});
...
Template.SeeDevUxorders.events({
	'click .closeBtn': ()=> {
		$(".panels").hide();
	},
